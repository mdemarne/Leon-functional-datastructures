...
/* Due to its specific use, this tree cannot be empty */
case class Tree(rank: BigInt,root: BigInt, children: List[Tree]) {
	...
	def link(that: Tree) : Tree = {
		...
		val res: Tree = (this, that) match {
			case (t1, t2) if t1.root <= t2.root => Tree(t1.rank + 1, t1.root, t2 :: t1.children)
			case (t1, t2) => Tree(t2.rank + 1, t2.root, t1 :: t2.children)
		}
		res
...
object Tree {
	...
	def apply(v: BigInt): Tree = Tree(0, v, Nil())
