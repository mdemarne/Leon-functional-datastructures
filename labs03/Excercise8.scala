...
object PackedSet {
  case class Range(min: BigInt, max: BigInt) {
    ...
    def <(that: Range) = {
      this.max + 1 < that.min // TODO: can we change that like that rather than min < min, to avoid overlap?
    }
  ...
  abstract class PackedSet {
    ...
    def +(v: BigInt): PackedSet = {
      require(this.isPackedSet) // TODO
      this match {
        /* General case */
        case Empty => NonEmpty(Range(v, v), Empty)
        case NonEmpty(r, tail) if r.contains(v) => this
        case NonEmpty(r1, NonEmpty(r2, tail)) if r1.max + 1 == v && v == r2.min - 1 => NonEmpty(Range(r1.min, r2.max), tail)
        case NonEmpty(r1, tail) if r1.min - 1 == v => NonEmpty(Range(v, r1.max), tail)
        case NonEmpty(r1, tail) if r1.min - 1 > v => NonEmpty(Range(v, v), NonEmpty(r1, tail))
        case NonEmpty(r1, tail) if r1.max + 1 == v => NonEmpty(Range(r1.min, v), tail)
        case NonEmpty(r1, tail) if r1.max + 1 < v => NonEmpty(r1, tail + v)
      }
    } ensuring {
    ...
    def -(v: BigInt): PackedSet = {
      ...
      this match {
        case Empty => Empty
        case NonEmpty(r, tail) if r.size == 1 && r.contains(v) => tail
        case NonEmpty(r, tail) if r.min == v => NonEmpty(Range(r.min + 1, r.max), tail)
        case NonEmpty(r, tail) if r.max == v => NonEmpty(Range(r.min, r.max - 1), tail)
        case NonEmpty(r, tail) if r.contains(v) => NonEmpty(Range(r.min, v - 1), NonEmpty(Range(v + 1, r.max), tail))
        case NonEmpty(r, tail) => NonEmpty(r, tail - v)
      }
    } ensuring {
      res => res.content == this.content -- Set(v) &&
        res.isPackedSet
